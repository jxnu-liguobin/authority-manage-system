...
public class BookController extends BaseController {
    ...
    return userService
        ...
        .all(
            user -> {
              ...
              if (bookIdList != null) {
                return bookIdList.parallelStream().filter(bId -> bId.equals(id)).count() <= 0;
              } else return true;
            })
        .flatMap(
            can -> {
              if (!can) {
                ...
                return bookService
                    .findByBookId(id)
                    .flatMap(temp -> deleteBook(temp, id, uCode))
                    .map(t -> JsonResult.success());
              }
            });
  }

  private Mono<Book> deleteBook(Book temp, String id, String uCode) {
    bookService
        .delete(id)
        .flatMap(
            t ->
                    ...
                    .flatMap(
                        u ->
                            memorandumComponent.saveMemorandum(
                                ...
                                u.getUserName(),
                                "删除图书",
                                temp.getBookId() + " | " + temp.getBookName())));
            ...
            bookService
                ...
                .flatMap(
                    b -> {
                      ...
                      // 更新库存
                      return bookService
                          .saveOrUpdate(book[finalI1])
                          .flatMap(bo -> borrowBookService.save(borrowBook[finalI1]));
                    })
                .log()
                .subscribe();
            // 添加到redis中
  ...
  public Mono<String> returnBookList(@PathVariable String id) {
    ...
    IntStream.range(0, books.length)
        .forEach(
            i -> bookService.findByBookId(borrowBooks[i].getBookId()).subscribe(b -> books[i] = b));
  ...
  public Mono<JsonResult> returnBook(@RequestBody BorrowList mBorrowList) throws Exception {
    ...
    while (i < mBorrowList.getBooklist().length) {
      ...
      bookService
          ...
          .map(
              book1 -> {
                book1.setCurrentInventory(book1.getCurrentInventory() + 1);
                return bookService.saveOrUpdate(book1);
              })
          .log()
          .subscribe();
  ...
  public String edit(@PathVariable String id, ModelMap map) {
    bookService.findByBookId(id).log().subscribe(b -> map.put("book", b));
    return "admin/books/addform";
  }
  ...
  public Mono<Page<Book>> findList(HttpServletRequest request) {
    ...
    return bookService.findAll(builder.generateSpecification(), getPageRequest(request)).log();
  }
}
