...
public class ShiroConfig {
    ...
    public ShiroFilterFactoryBean getShiroFilterFactoryBean(
            SessionsSecurityManager securityManager, Realm realm) {
        ...
        // 注册账号验证和添加账号验证
        filterChainDefinitionMap.put("/admin/user/notExist/**", "anon");
        ...
        filterChainDefinitionMap.put(
                "/admin/resource/deleteBatch", "perms[system:resource:deleteBatch]");

        filterChainDefinitionMap.put("/druid/", "perms[system:resource:druid]"); // druid
        filterChainDefinitionMap.put(
                "/admin/memorandum/*", "perms[system:memorandum:memorandum]"); // 系统记录,只使用一个拦截url
        // 添加过滤条件
        filterChainDefinitionMap.put(
                "/admin/books/book_management", "perms[system:books:book_management]");

        filterChainDefinitionMap.put(
                "/admin/**", "user"); // 默认所有均可依靠cookie,本项目隐藏bug，cookie太大，无法保存在浏览器本地
